...
import issueDetails from "../../static/img/docs/platform/issue-details-page.png";
import issueThreadsPanel from "../../static/img/docs/platform/issue-threads-panel.png";
...
<ImageFigure
  src={issueDetails}
  alt="Issue Details page of the Memfault Web App"
  title="Issue Details"
>
  Issue Details page of the Memfault Web App.
</ImageFigure>
...
The panel contains a list of threads used to trace back the event log of each individual process associated with the core dump.

<ImageFigure
  src={issueThreadsPanel}
  alt="Threads Panel on the Memfault Web App"
  title="Threads Panel"
>
  Threads Panel on the Memfault Web App
</ImageFigure>

### Call Stack
The panel consists of the call stack, which can be opened to view it's children processes. 
By following the tree-like structure you're able to dive deeper into the cause of the issue.

#### Functions
The call stack contains a list of processes for each function. Open the function to reveal more details on the memory address, file location, frame size, type, and source. 
...
State tags are useful for determining which process to start inspecting first. You're more likely to find an issue in a suspended or running process, rather than a ready or blocked process.
...
- Assert: An caught error created during testing within a service.
- Watchdog: A validation error related to outdated or corrupted files in your device.
- Hard Fault: A serious error that causes slow down or thrashing due to the device taking more time to return results.
- Mem Fault: An error thrown by hardware for memory protection. This occurs when a program writes to a restricted area.

Causal tags are used to assist in the debugging process and provide a high-level description of what error occured on the device, as well the severity.
## Issue Panel
